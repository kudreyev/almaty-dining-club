...
export class DataController {
    async query(req, res) {
        ...
        const table = tableMap[body.table];
        if (!table) {
        ...
        if (body.action === 'select') {
            ...
            if (body.filters?.length) {
                const filters = body.filters.map((f) => f.op === 'eq' ? eq(table[f.column], f.value) : gt(table[f.column], f.value));
                builder = builder.where(and(...filters));
            }
            if (body.orderBy) {
                builder = builder.orderBy(body.orderBy.ascending ? asc(table[body.orderBy.column]) : desc(table[body.orderBy.column]));
            }
            if (body.limit) {
        ...
        if (body.action === 'insert') {
            const insertedRows = (await db.insert(table).values(body.values ?? {}).returning());
            const row = insertedRows[0] ?? null;
            return res.status(200).json({ ok: true, data: row });
        }
        if (body.action === 'update') {
            let builder = db.update(table).set(body.values ?? {});
            if (body.filters?.length) {
                const filters = body.filters.map((f) => f.op === 'eq' ? eq(table[f.column], f.value) : gt(table[f.column], f.value));
                builder = builder.where(and(...filters));
            }
