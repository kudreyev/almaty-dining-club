...
class QueryBuilder<T = any> implements PromiseLike<any> {
  ...
  select<U = T>(_columns?: string): QueryBuilder<U> { this.action = 'select'; return this as unknown as QueryBuilder<U> }
  insert(values: Record<string, unknown> | Record<string, unknown>[], ..._args: any[]) { this.action = 'insert'; this.values = Array.isArray(values) ? (values[0] ?? {}) : values; return this }
  ...
  not(..._args: any[]) { return this } or(..._args: any[]) { return this } in(..._args: any[]) { return this } is(..._args: any[]) { return this } gte(..._args: any[]) { return this }
  returns<U = T>(): QueryBuilder<U> { return this as unknown as QueryBuilder<U> }
  upsert(values: Record<string, unknown> | Record<string, unknown>[], ..._args: any[]) { this.action = 'insert'; this.values = Array.isArray(values) ? (values[0] ?? {}) : values; return this }
  ...
  private async execute<U = T>() {
    try {
      ...
      const response = await fetch(`${API_URL}/api/data/query`, {
        ...
        body: JSON.stringify({ table: this.table, action: this.action, filters: this.filters, orderBy: this.orderBy, limit: this.limitValue, values: this.values, single: this.singleRow }),
        cache: 'no-store',
      })
      ...
      return { data: null, error: { message: 'API unavailable during build/runtime' } as Error }
    }
  }
...
async function getUserWithCookie() {
  try {
    const cookieStore = await cookies()
    const cookieHeader = cookieStore.getAll().map((c) => `${c.name}=${c.value}`).join('; ')
    const response = await fetch(`${API_URL}/api/auth/me`, { method: 'GET', headers: cookieHeader ? { Cookie: cookieHeader } : undefined, cache: 'no-store' })
    ...
    return { data: { user: null }, error: { message: 'API unavailable during build/runtime' } }
  }
}

export function createCompatServerClient() {
  return {
