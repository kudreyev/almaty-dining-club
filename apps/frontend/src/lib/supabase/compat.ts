...
class QueryBuilder<T = any> implements PromiseLike<any> {
  ...
  constructor(
    ...
    private readonly withAuthCookies: boolean
  ) {}
  ...
  private async execute<U = T>() {
    const cookieStore = this.withAuthCookies ? await cookies() : null
    ...
    if (!response.ok) {
      ...
      return { data: null, error: { message } as Error }
    }
...
export function createCompatClient(withAuthCookies = false) {
  return {
    from<U = any>(table: string): QueryBuilder<U> {
      return new QueryBuilder<U>(table, withAuthCookies)
    },
    auth: {
      async getUser() {
        return (await getUserWithCookie()) as any
      },
      admin: {
        async createUser(args: { email: string }) {
          return {
            data: { user: { id: crypto.randomUUID(), email: args.email } },
            error: null,
          }
        },
        async generateLink() {
      ...
      async verifyOtp(..._args: any[]) {
        return { data: null, error: { message: 'verifyOtp moved to backend auth flow' } }
      },
      async exchangeCodeForSession(..._args: any[]) {
        return { error: { message: 'exchangeCodeForSession removed' } }
      },
    },
    storage: {
      from() {
        return {
          getPublicUrl(path: string) {
            return {
              data: {
                publicUrl: `${process.env.NEXT_PUBLIC_API_URL}/files/${path}`,
              },
            }
          },
          async upload(..._args: any[]) {
            return { error: { message: 'Storage upload moved to backend' } }
          },
      ...
      async getBucket(..._args: any[]) {
        return { data: null, error: { message: 'Bucket check moved to backend' } }
      },
